...
const signupUrl = "http://localhost:5173";
...
async function createAccount(account) {
  ...
  try {
    ...
    try {
      await page.waitForNavigation({ waitUntil: "networkidle2", timeout: 200 });
    } catch (navErr) {
...
// Function to rotate IP using a DHCP renewal (Linux only)
async function rotateIP() {
  return new Promise((resolve, reject) => {
    // You may need to adjust the command based on your network configuration
    exec("sudo dhclient -r && sudo dhclient", (error, stdout, stderr) => {
      if (error) {
        console.error("IP rotation failed:", stderr);
        return reject(error);
      }
      console.log("IP rotation successful:", stdout);
      // Optionally wait a few seconds for the network to stabilize
...
(async () => {
  for (const account of accounts) {
    ...
    try {
      ...
      // Attempt to rotate IP after each account registration
      await rotateIP();
